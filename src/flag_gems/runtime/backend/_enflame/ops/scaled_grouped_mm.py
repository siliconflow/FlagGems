...
def _scaled_grouped_mm_kernel(
    ...
    flat_tile = tl.program_id(0).to(tl.int64)
    # GCU300 grid limits are x <= 65535 and y <= 255. Flatten and stride the
    # work rather than placing a potentially large tile count on grid.y.
    while flat_tile < NUM_GROUPS * TILES_PER_GROUP:
        group_idx = (flat_tile // TILES_PER_GROUP).to(tl.int64)
        tile_idx = (flat_tile % TILES_PER_GROUP).to(tl.int64)
        ...
        if A_IS_2D or B_IS_2D:
            offset_start = tl.load(
                offs + group_idx - 1, mask=group_idx > 0, other=0
            ).to(tl.int64)
        ...
        # Avoid the compiler's signed-int64 range lowering (tops_abs_si64).
        k_offset = 0
        while k_offset < k_size:
        ...
        flat_tile += tl.num_programs(0).to(tl.int64)
...
def scaled_grouped_mm(
    ...
    logger.debug("GEMS_ENFLAME SCALED_GROUPED_MM")
    ...
    tiles_per_group = max_m_tiles * max_n_tiles
    grid = (min(num_groups * tiles_per_group, 65535),)
